...
*   **规则与偏好的抽象化与持久化**:
    *   **严禁项目名称**: 在 `docs/rules/` 和 `docs/prefs/` 的文档内容中，禁止出现任何 `data/` 下具体的项目名称或特定源码引用。必须将模式抽象为“通用的最佳实践”。
    *   **增量更新**: 除非必要，**禁止删除** `docs/prefs/` 内的现有内容，仅允许添加。
    *   **谨慎变更**: 若有充分理由需要删除或重构偏好，必须告知用户理由并在获得明确同意后执行。
